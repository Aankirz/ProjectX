...
const Marketplace = () => {
  const { getLoanProviders, allLoanProviders } = useContext(ContractContext);
  useEffect(() => {
    getLoanProviders();
    console.log(allLoanProviders);
  }, []);
  return (
            ...
            <div className="flex font-bold bg-gray-800 m-8 border border-gray-300 rounded-lg p-3 px-20">
              <div className="flex flex-col">
                <div className="text-4xl font-bold">
                  {" "}
                  {quote.type === "borrow" ? "Borrow" : "Lend"} Request{" "}
                </div>
                <div className="relative top-4 font-bold text-xl">
                  {" "}
                  Amount: {quote.amount}{" "}
                </div>
                <div className="relative top-8 p-2 px-4 ml-3 text-gray-100 font-bold text-xl rounded-lg z-10 w-fit border border-gray-900 bg-gray-500 ">
                  {quote.username}
                </div>
              </div>
              <div className="m-9">Interest : {quote.interest}%</div>
              <div className="relative top-8 p-2 px-4 ml-3 font-bold text-xl rounded-lg  h-12 border border-gray-900 bg-gray-600 ">
